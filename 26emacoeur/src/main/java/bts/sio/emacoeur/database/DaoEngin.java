...
public class DaoEngin {
    ...
    ArrayList<Engin> desEngins = new ArrayList<>();
    try {
        requete = cnx.prepareStatement(
            "SELECT eng_id as e_id, eng_type as e_type FROM engin"
        );
...
public static Engin getEnginById(Connection cnx, int idEngin) {
    ...
    try {
        requete = cnx.prepareStatement(
            "SELECT eng_id as e_id, eng_type as e_type FROM engin WHERE eng_id = ?" // ✅ WHERE ajouté
        );
